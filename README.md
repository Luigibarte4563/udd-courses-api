...
| `GET /data/courses/{id}.json` | One program by ID, with its school details |
...
```
https://<your-username>.github.io/udd-courses-api/
```
...
### Option B — Deploy a subfolder with Actions (advanced)
...
Then enable Pages under **Settings → Pages** with **Source: GitHub Actions**.
...
```js
const response = await fetch(
  "https://<your-username>.github.io/udd-courses-api/data/courses/102.json"
);

const course = await response.json();

console.log(course.name); // BS Information Technology - Web Development
```
